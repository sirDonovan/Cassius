...
let commands = {
	...
	mail: function (target, room, user) {
		...
		if (to in database.mail) {
			...
			if (queued >= 3) return this.say("You have too many messages queued for " + Users.get(targets[0]).name + ".");
		} else {
			database.mail[to] = [];
		}
		database.mail[to].push({time: Date.now(), from: user.name, text: message});
		Storage.exportDatabase('global');
		this.say("Your message has been sent to " + Users.get(targets[0]).name + "!");
	},
